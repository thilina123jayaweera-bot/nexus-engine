...
function computeRSIbubble(sym){
  ...
  // short-term momentum: % change over ~the last 3 minutes (falling-knife detector).
  const now=b[b.length-1].t, cutoff=now-180000;
  const recent=b.filter(x=>x.t>=cutoff);
  if(recent.length>=2){ const first=recent[0].p, last=recent[recent.length-1].p;
    if(first>0) t.mom=((last-first)/first)*100; }
}
...
async function bootstrapUniverse(log){
  try{
    ...
    TK=syms.map(s=>({s, funding:null, oi:null, rsi:null, bubble:null, mvrvZ:null, netflow:null,
      smartMoney:null, socialVol:null, galaxyScore:null, tieScore:null, ofi:null, liqL:null, liqS:null, h24:8, mom:null}));
