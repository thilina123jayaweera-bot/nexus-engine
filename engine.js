...
async function manageExits(){
  ...
  for(const tr of S.trades){ if(tr.status==='CLOSED')continue;
    ...
    tr.aiEval=ev;
    if(['STOP OUT','CAP STOP','CLOSE TP','SPIKE TAKE','FAST SPIKE TAKE','PEAK-FADE TAKE','GIVE-BACK EXIT','DECAY CUT','EARLY EXIT','STAGNATION EXIT','CLOSE FLIP','CLOSE STALE'].includes(ev.action) || ev.action.startsWith('CLOSE ')){
