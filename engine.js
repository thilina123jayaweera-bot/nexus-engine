...
async function manageExits(){
  ...
  for(const tr of S.trades){ if(tr.status==='CLOSED')continue;
    ...
    tr.aiEval=ev;
    if(['STOP OUT','CAP STOP','CLOSE TP','SPIKE TAKE','FAST SPIKE TAKE','GIVE-BACK EXIT','EARLY EXIT','CLOSE FLIP','CLOSE STALE'].includes(ev.action) || ev.action.startsWith('CLOSE ')){
