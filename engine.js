...
async function scanAndEnter(){
  ...
  const cands=[]; let scanned=0, noData=0, best=null;
  for(const t of md.getTK()){
    scanned++;
    if(!md.hasLiveData(t.s)){ noData++; continue; }
    const f=D.applyFilters(t, ctxBase());
    // track the closest-to-passing token for the diagnostic
    if(!best || f.ps.total>best.score) best={sym:t.s, dir:f.direction, score:f.ps.total|0,
      fail:(f.filters.find(x=>!x.pass)||{}).label||'passed'};
    if(f.pass) cands.push({t,f,score:f.ps.total});
  }
  // ALWAYS report the scan result — turns silence into a clear answer
  if(cands.length===0){
    log(`SCAN: ${scanned} tokens, ${noData} no-data, 0 qualified · top: ${best?`${best.sym} ${best.dir} score ${best.score} (fail: ${best.fail})`:'n/a'} · F&G ${md.MKT.fearGreed} ${md.CORR.regimeStress}`,'🔎');
    return;
  }
  cands.sort((a,b)=>b.score-a.score);
  log(`SCAN: ${scanned} tokens, ${cands.length} qualified → entering top ${Math.min(cands.length,cfg.MAX_POS-open)}`,'🔎');
