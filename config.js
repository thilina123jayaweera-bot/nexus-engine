...
module.exports = {
  ...
  RULES:{
    ...
    TRAIL_START_PCT:0.5, TRAIL_LOCK:0.70, SCORE_DROP_EXIT:15, ROTATE_ADVANTAGE:25,
    ...
    GIVEBACK_MIN_PEAK_ROI:6,    // arm only after peak ROI ≥ this % (6: let winners run toward TP first)
    ...
    STREAK_DECAY_MIN:90,        // decay lossStreak by 1 after this many minutes without a new material loss
    ...
    // ── "Don't let trades rot" — bank faded peaks, free stagnant slots ────────
    PEAKFADE_MIN_ROI:3,         // peak-fade: only after a real peak ≥ this %
    PEAKFADE_KEEP:0.6,          // … fire once ROI drops below 60% of peak …
    PEAKFADE_STALL_MIN:15,      // … and no new high in this many minutes (peak is behind us)
    STAGNATION_MIN:150,         // stagnation: after this many minutes held …
    STAGNATION_BAND:1.5,        // … if |ROI| < this and it never really peaked, free the slot
    FATIGUE_SPAN:20,            // fatigue-trail: score-drop (pts) that fully tightens the trail
    // Entry quality in weak tape — don't open fresh longs into active weakness.
    ENTRY_BTC_DRIFT:-0.15,      // skip new LONGs if BTC is drifting down worse than this %/min
    ENTRY_KNIFE_MOM:-1.2,       // skip new LONGs if the token itself fell more than this % in ~3 min
    // Early-deterioration cut — mirror of peak-fade, for losers that NEVER worked.
    // Cut a modestly-underwater trade that never got into profit and isn't bouncing,
    // instead of bleeding it to the -5% EARLY EXIT / -8% STOP OUT.
    DECAY_CUT_ROI:2.0,          // trigger once ROI ≤ -this %
    DECAY_NEVER_PEAK:1.0,       // … and peak ROI never exceeded this % (it never worked)
    DECAY_MIN_HOLD:20,          // … and it's had at least this many minutes to work
    DECAY_SPARE_MOM:0.15,       // … spared only if the token is actively bouncing (mom > this %)
