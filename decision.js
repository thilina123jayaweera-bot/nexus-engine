...
// Each bucket is a proper 0-100 sub-score (neutral ~45, strong ~85), so the
// weighted blend spans the full 0-100 range and the 72 threshold is reachable.
function dualScore(t, MKT, CORR){
  const clamp=(x)=>Math.max(0,Math.min(100,x));
  const fg=MKT.fearGreed||50;
  // Derivatives (positioning / leverage stress)
  let LD=45,SD=45;
  LD+= t.funding<0.005?25:t.funding<0.02?12:t.funding<0.05?0:-20;
  SD+= t.funding>0.05?25:t.funding>0.02?12:t.funding>0.005?0:-15;
  const liqTot=(t.liqL||0)+(t.liqS||0)+1;
  LD+= (((t.liqS||0)-(t.liqL||0))/liqTot)*20; SD+= (((t.liqL||0)-(t.liqS||0))/liqTot)*20;
  LD+= (t.ofi||0)>1?15:(t.ofi||0)>0?7:0;      SD+= (t.ofi||0)<-1?15:(t.ofi||0)<0?7:0;
  // On-chain (holder behaviour / valuation)
  let LC=45,SC=45;
  LC+= t.netflow<-50?25:t.netflow<0?12:t.netflow>50?-15:0;
  SC+= t.netflow>50?25:t.netflow>0?12:t.netflow<-50?-15:0;
  LC+= t.mvrvZ<0.5?25:t.mvrvZ<1?15:t.mvrvZ<2?5:t.mvrvZ>3?-15:-5;
  SC+= t.mvrvZ>3?25:t.mvrvZ>2?15:t.mvrvZ>1?5:-10;
  LC+= ((t.smartMoney||0.5)-0.5)*30;          SC+= (0.5-(t.smartMoney||0.5))*30;
  // Sentiment (contrarian at extremes)
  let LS=40,SS=40;
  LS+= fg<20?38:fg<30?28:fg<45?15:fg<55?5:-10;    // deep fear → strong long
  SS+= fg>80?38:fg>70?28:fg>55?15:fg>45?5:-10;    // greed → strong short
  LS+= (t.galaxyScore||50)>65?12:(t.galaxyScore||50)>50?5:0;
  SS+= (t.galaxyScore||50)<40?12:0;
  // Technical
  let LE=45,SE=45;
  const rsiOk=Number.isFinite(t.rsi)&&t.rsi>=3&&t.rsi<=97;
  LE+= rsiOk?(t.rsi<25?38:t.rsi<35?28:t.rsi<45?15:t.rsi<60?2:-12):0;
  SE+= rsiOk?(t.rsi>75?38:t.rsi>65?28:t.rsi>55?15:t.rsi>40?2:-12):0;
  LE+= (t.bubble||50)<35?15:(t.bubble||50)<50?6:(t.bubble||50)>75?-12:0;
  SE+= (t.bubble||50)>65?15:(t.bubble||50)>50?6:(t.bubble||50)<25?-12:0;
  ...
  // BTC regime multiplier (the biggest lever) — only bites on active BTC moves
  if(CORR){
    const m=CORR.btcMoveLastMin||0;
    if(CORR.regimeStress==='EXTREME'&&m<-0.5) longTotal*=0.35;
    else if(m<-0.15) longTotal*=0.80;
