...
function dualScore(t, MKT, CORR){
  ...
  // OVER-EXTENSION SHORT PATH: a clearly crowded-long / overbought / stretched token
  // can short even in a fear regime, where the market-wide sentiment tilt would
  // otherwise force everything long. Needs ≥2 flags — kept rare and honest. (Short
  // quality is data-limited until Coinglass adds OI/liquidations — see TODO below.)
  const rsiOk2=Number.isFinite(t.rsi)&&t.rsi>=3&&t.rsi<=97;
  const overext = (has(t.funding)&&t.funding>0.05?1:0) + (rsiOk2&&t.rsi>70?1:0) + (has(t.bubble)&&t.bubble>75?1:0);
  // TODO(coinglass): add crowded-OI + long-liquidation-cascade flags here (the real short edge)
  if(overext>=2) shortTotal += overext*RULES.SHORT_OVEREXT_BOOST;
...
function applyFilters(t, ctx){
  ...
  // Adaptive bar after losses — capped small: +8 made the bar unreachable (scores
  // cluster ~76 on the current data), locking the engine out for hours.
  const thr=RULES.MIN_PROFIT_SCORE+(portfolio.lossStreak>=5?6:portfolio.lossStreak>=3?3:0);
  ...
  if(CORR){
    ...
    // ENTRY QUALITY: don't buy fresh dips into active weakness (the EARLY-EXIT losers).
    if(dir==='LONG' && CORR.btcMoveLastMin<RULES.ENTRY_BTC_DRIFT)
      G(`BTC drift ${CORR.btcMoveLastMin.toFixed(2)}>${RULES.ENTRY_BTC_DRIFT}`, false);
  }
  if(dir==='LONG' && Number.isFinite(t.mom) && t.mom<RULES.ENTRY_KNIFE_MOM)
    G(`KNIFE ${t.mom.toFixed(1)}%>${RULES.ENTRY_KNIFE_MOM}`, false);   // still actively falling → skip
...
function computeEntrySize(t, f, ctx){
  ...
  const shortMult=(f.direction==='SHORT')?RULES.SHORT_SIZE_HAIRCUT:1;   // shorts smaller (lower conviction)
  margin=margin*(f.sizeMult||1)*regimeMult*shortMult;
...
function evalTrade(tr, ctx){
  ...
  // ── SPIKE TAKE — grab a sharp ROI spike immediately (speed > confirmation) ──
  // Sudden pops mean-revert before the lagging give-back can confirm, so bank them now.
  const peak=tr.maxRoi||0;
  if(pnl>=RULES.SPIKE_ROI) return A('SPIKE TAKE',`ROI +${pnl.toFixed(1)}% spike`,true);
  { const hist=tr._roiHist||[]; const cutoff=Date.now()-RULES.FAST_SPIKE_WIN_MS;
    const past=hist.filter(h=>h.t>=cutoff);
    if(past.length>=2){ const lowRecent=Math.min(...past.map(h=>h.roi));
      if(pnl>=RULES.FAST_SPIKE_ROI && (pnl-lowRecent)>=RULES.FAST_SPIKE_ROI)
        return A('FAST SPIKE TAKE',`+${(pnl-lowRecent).toFixed(1)}% in ${(RULES.FAST_SPIKE_WIN_MS/60000)|0}m`,true); } }
  // ── MOMENTUM-ROLLOVER GIVE-BACK EXIT (asymmetric decay) ──────────────────
  // A trade that banked a profit and has rolled over past a peak-scaled give-back
  // threshold, WITH momentum turning, exits now at a thin margin rather than riding
  // to the stop. Small peaks get little room (bank the win); big peaks get more
  // room (let winners breathe). Uses only ROI + score/RSI, which we have.
  if(peak>=RULES.GIVEBACK_MIN_PEAK_ROI){
    const hi=Math.max(RULES.GIVEBACK_PEAK_HIGH_ROI, RULES.GIVEBACK_MIN_PEAK_ROI+1);
    const f=Math.min(1,Math.max(0,(peak-RULES.GIVEBACK_MIN_PEAK_ROI)/(hi-RULES.GIVEBACK_MIN_PEAK_ROI)));
    const keepFrac=RULES.GIVEBACK_TIGHT+(RULES.GIVEBACK_WIDE-RULES.GIVEBACK_TIGHT)*f;  // 0.70 → 0.35 as peak grows
    const trigger=peak*keepFrac;
    // rollover confirmation from live signals — scoreWeak or RSI exhaustion only.
    // (opp>es removed: with clustered scores it fired on noise and strangled winners.)
    // TODO(coinglass): add order-flow toxicity → (cvdDelta < -X && oiDelta < 0).
    const rolled = scoreWeak || exhausted;
    if(pnl<trigger && rolled) return A('GIVE-BACK EXIT',`peak +${peak.toFixed(1)}% → +${pnl.toFixed(1)}% rollover (keep ${(keepFrac*100)|0}%)`,true);
  }
  // ── DECAY CUT — cut a never-worked loser early (mirror of PEAK-FADE TAKE) ──
  // Modestly underwater, never got into profit, has had time to work, and isn't
  // bouncing → cut at ~-2% instead of bleeding to the -5%/-8% stops.
  if(pnl<=-RULES.DECAY_CUT_ROI && (tr.maxRoi||0)<RULES.DECAY_NEVER_PEAK && holdMin>RULES.DECAY_MIN_HOLD
     && !(Number.isFinite(t.mom) && t.mom>RULES.DECAY_SPARE_MOM))
     return A('DECAY CUT',`${pnl.toFixed(1)}% never worked, not bouncing — cut early`,true);
  if(pnl<-ctx.stopLoss*lev*0.55 && scoreWeak)        return A('EARLY EXIT',`losing + score dropped`,true);
  // ── PEAK-FADE TAKE — bank a real peak that has faded AND stalled ──────────
  // Catches "was up +6%, drifted to +3%, not recovering" even when the score
  // stays clustered/OK (so give-back's momentum test never fires) — your case.
  if(peak>=RULES.PEAKFADE_MIN_ROI && pnl>0 && pnl<=peak*RULES.PEAKFADE_KEEP){
    const hist=tr._roiHist||[]; const stallCut=Date.now()-RULES.PEAKFADE_STALL_MIN*60000;
    const recentHigh=Math.max(pnl, ...hist.filter(h=>h.t>=stallCut).map(h=>h.roi));
    if(recentHigh < peak*0.98)   // no new high in the stall window → the peak is behind us
      return A('PEAK-FADE TAKE',`peak +${peak.toFixed(1)}% → bank +${pnl.toFixed(1)}% (stalled ${RULES.PEAKFADE_STALL_MIN}m)`,true);
  }
  // ── STAGNATION EXIT — free a slot from a long hold going nowhere ──────────
  // The 400-min bleeders that never really worked and are blocking better setups.
  if(holdMin>RULES.STAGNATION_MIN && Math.abs(pnl)<RULES.STAGNATION_BAND && peak<RULES.PEAKFADE_MIN_ROI)
    return A('STAGNATION EXIT',`${holdMin|0}m flat (peak +${peak.toFixed(1)}%) — freeing slot`,true);
  // CLOSE FLIP only when this position's OWN thesis has broken (scoreWeak) AND the
  // opposite side is substantially stronger — not on mild score drift. Flat, still-OK
  // positions are left for STAGNATION EXIT instead of being churned at breakeven.
  if(scoreWeak && opp.total-es.total>RULES.ROTATE_ADVANTAGE && pnl<1)
    return A('CLOSE FLIP',`thesis broke + opposite +${(opp.total-es.total)|0}`,true);
  if(holdMin>RULES.MAX_HOLD_MINUTES && Math.abs(pnl)<1) return A('CLOSE STALE',`flat ${holdMin|0}m`,true);
  // ── PYRAMID ADD — add to a strong, still-rising winner (add to what works) ──
  // Fires only while the trade is up, its score is intact, and it hasn't rolled
  // over. A weakening winner falls through to PARTIAL TAKE (bank) instead.
  if(RULES.PYRAMID && !tr.pyramided && pnl>=RULES.ADD_TRIGGER_ROI
     && !scoreWeak && !exhausted && es.total>=(tr.entryScore||60)-2
     && pnl>=(tr.maxRoi||0)*RULES.ADD_MIN_PEAK_KEEP)
     return A('PYRAMID ADD',`+${pnl.toFixed(1)}% & strong (score ${es.total|0})`,false);
  ...
  if(pnl>=RULES.TRAIL_START_PCT*lev && !scoreWeak){
    // fatigue-weighted trail: tighten the lock as the live score fades from entry,
    // so a weakening winner banks more of its gain (the one AI idea worth taking).
    const fatigue=Math.max(0,Math.min(1,((tr.entryScore||70)-es.total)/RULES.FATIGUE_SPAN));
    const lock=RULES.TRAIL_LOCK+(1-RULES.TRAIL_LOCK)*fatigue;
    return { ...A('TRAIL STOP',`trail lock ${(lock*100)|0}%${fatigue>0?` (fatigue ${(fatigue*100)|0}%)`:''}`,false), trailLock:lock };
  }
