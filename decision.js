...
function evalTrade(tr, ctx){
  ...
  if(peak>=RULES.GIVEBACK_MIN_PEAK_ROI){
    ...
    // rollover confirmation from live signals. TODO(coinglass): replace/augment with
    // order-flow toxicity → (cvdDelta < -X && oiDelta < 0) once those feeds exist.
    const rolled = scoreWeak || exhausted || opp.total>es.total;
